		...
		for j in range(2):
			for k in range(2):
				...
				for l in range(self.N_orient):
					...
					self.surf_concs[l,j,k] = ((self.concs[j,k]+ s_rel)/( s_up))
		...
		for j in range(2):
			for k in range(2):
				...
				s_up = np.dot(self.uptake_coeffs[j][k],
					self.uptake_concs[j][k])
				self.surf_concs[j,k] = ((self.concs[j,k] +s_rel)/( s_up))
		...
		if self.enzymes_per_vesicle == 2 and self.surf_conc_mode != 'uniform':
			for k in range(2):
				if j in self.uptake_transitions[0][k]:
					self.expanded_coeff[0] *= self.surf_concs[0,0,k]
					self.expanded_coeff[1] *= self.surf_concs[1,1,k]
					return
				elif j in self.uptake_transitions[1][k]:
					self.expanded_coeff[0] *= self.surf_concs[0,1,k]
					self.expanded_coeff[1] *= self.surf_concs[1,0,k]
					return
		else:
			for k in range(2):
				if j in self.uptake_transitions[0][k]:
					self.expanded_coeff[0] *= self.surf_concs[0,k]
					self.expanded_coeff[1] *= self.surf_concs[1,k]
					return
				elif j in self.uptake_transitions[1][k]:
					self.expanded_coeff[0] *= self.surf_concs[1,k]
					self.expanded_coeff[1] *= self.surf_concs[0,k]
					return
